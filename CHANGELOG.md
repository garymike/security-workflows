...
- **`docs/threat-model.md`: corrected the SkillSpector overclaims fixed in 1.3.1** (this doc was missed then):
  "the surface no published scanner covers" became "scanners advise; the gate enforces (exit 1)."
...
- **`security-scan.yml` reusable: three parallel jobs collapsed into one.** The former
  ...
  1-minute floor and re-ran `checkout` on every caller, about 3 billable minutes for seconds
...
- **Local runner: the gate on any OCI runtime, no Docker Desktop required.** A new wrapper
  ...
  (`docker`, then `podman`, then WSL Containers `wslc`), health-checking each (installed ≠ running), and
...
- **skill-testfile-gate v2: the developer-execution surface, deepened.** The gate now separates
  **presence** (an auto-executed skill file exists, low-severity inventory) from **malice** (that
  ...
  false-positived. See ADRs [0010](docs/adr/0010-first-party-dev-exec-rule-pack.md) to [0012](docs/adr/0012-layered-severity-and-sarif.md).
...
- **CI proof-fixture** (`tests/gate-proof.sh`, wired into `dogfood-scan`): asserts the gate blocks a
  defanged Gecko demo and clears a benign skill on every build, so the distinction is proven continuously.
...
- **Signed images**: `security-toolbox-base`, then `mcp-review-toolbox`, `gha-toolbox`, `skill-audit-toolbox`, `sast-toolbox`, `iac-toolbox` (each SBOM'd, provenance-attested, Trivy-gated, cosign-signed).
...
- `sast-toolbox` (base + **Semgrep OSS**) + reusable `sast.yml` (SARIF to code scanning) and reusable `codeql.yml` (CodeQL, the deep engine for public repos): defense-in-depth SAST that closes issue #2's SAST and #3's Trusys (Semgrep carries the LLM rulesets). CodeQL is also recommended via GitHub's code-scanning default setup.
- `iac-toolbox` (base + **Checkov**) + reusable `iac-security.yml`: Infrastructure-as-Code / misconfiguration scanning (Terraform, Dockerfiles, K8s, Helm, CloudFormation), SARIF to code scanning. Closes issue #2's IaC. (KICS rejected as redundant, see `docs/tool-evaluations.md`.)
...
- `gha-toolbox` image (base + **zizmor** + **actionlint** + shellcheck) and the reusable **`gha-security.yml`** workflow that audits GitHub Actions workflows for expression/template injection, excessive permissions, unpinned actions, and shell issues. Built via the domain matrix and dogfooded in `dogfood-scan.yml`.
- `skill-audit-toolbox` image (base + **SkillSpector**, pinned by commit + a first-party **test-file gate**) and the reusable **`skill-audit.yml`** workflow that reviews agent skills across the agent-execution surface (SkillSpector) and the developer-execution / test-file surface no scanner covers. Adds **`docs/threat-model.md`** documenting the coverage map, citations, and residual gaps.
...
- Documentation: `docs/adr/` (architecture decision records 0001 to 0009), `docs/architecture.md` (three-plane overview + layer graph), and `docs/tool-evaluations.md` (the tool-selection ledger).
...
- `security-scan.yml` (reusable) now runs secret scanning with **betterleaks from the pinned `security-toolbox-base` image** instead of `gitleaks-action`, one pinned source of truth. Input renamed from `gitleaks-fail-on-findings` to `fail-on-findings`; the secrets job now needs `packages: read` to pull the image.
- Composite action `actions/mcp-review-toolbox` renamed to `actions/toolbox-scan` (generalized; default image is now `security-toolbox-base`).
