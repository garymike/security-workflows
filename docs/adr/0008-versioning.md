...
## Context
Consumers referenced the reusable workflows at `@main` — silent breakage, not
publishable-grade.
...
## Consequences
Self-consistency note: this repo's own action-pinning check flags `@vN` as unpinned, so
the *primary* guidance is SHA-pinning (matching the linter), with the moving major
offered as a stated-tradeoff convenience.
