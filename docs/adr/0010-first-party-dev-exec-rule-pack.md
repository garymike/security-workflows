...
filename-only gate fires on every honest test suite. Distinguishing a benign test from a
weaponized one needs content analysis. [ADR-0004](0004-aggregator-not-a-fork.md) says we aggregate
established tools rather than build and maintain detection logic, but no published tool covers
this surface (that is the gap this repo fills), and the state-of-the-art study
(arXiv 2601.10338, *SkillScan*) scopes itself to "`SKILL.md` + scripts the skill may invoke", the
agent-execution path, which excludes the developer-execution surface by construction.
...
there is nothing to aggregate. It reuses the Semgrep engine already shipped in sast-toolbox, so it
honours ADR-0004's spirit: reuse the engine and author rules rather than build a parser. Semgrep is pinned
explicitly in `skill-audit-toolbox` (not left to SkillSpector's transitive pull).

## Consequences
We own about nine rules, deliberately table-stakes primitives (credential reads, `curl|bash`,
...
SFS Packing bypasses >90% of nine scanners), so the pack is a cheap pre-filter rather than a trust gate
