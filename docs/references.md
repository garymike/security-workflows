...
- **[SkillScan]** Liu, Wang, Feng, Zhang, Xu, Deng, Li, Zhang — *Agent Skills in the Wild: An Empirical
  ...
  taxonomy. **We cite it for:** the scanner-scope boundary ("`SKILL.md` + scripts the skill *may invoke*")
...
- **[SkillCloak] / [SkillDetonate]** Ji, Xu, Li, Gao, Wei, Wang, Cheung (HKUST) — *Cloak and Detonate:
  ...
  scanners**; SkillDetonate (sandboxed, eBPF+FUSE taint) catches 97%. **We cite it for:** static analysis
  being a cheap pre-filter, not a trust gate (→ the escalate-to-sandbox ladder); the blind-dir table
...
- **[MalSkillBench]** Guo, Zeng, Liu, Jia, Xu, Tang, Fang, Liu — *MalSkillBench: A Runtime-Verified
  ...
  — 3,944 malicious / 4,000 benign, 108-cell taxonomy. **We cite it for:** the "hybrid artifact" result
  (naive OR-combining detectors over-triggers — up to 3,979 FPs on 4,000 benign → use layered *severity*).
  **Access note:** practical access routes through the commercial Kirin platform (<https://www.getkirin.com>);
