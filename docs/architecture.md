...
`security-workflows` is a **best-in-class aggregator** of upstream security tooling,
organized in three planes over a hardened, pinned supply chain.

## Three planes

1. **Reusable workflows** (`workflow_call`) — the public interface other repos call:
   - `security-scan.yml` — secret scanning (betterleaks, via the pinned image) + action-pinning + SECURITY.md checks
   - `security-audit.yml` — repo posture (Dependabot, Actions permissions, branch protection, visibility intent)
   - `gha-security.yml` — GitHub Actions security (zizmor + actionlint)
   - `skill-audit.yml` — agent-skill review (SkillSpector + the developer-execution `skill-testfile-gate`)
2. **Domain toolbox images** — signed, use-case-scoped containers; the execution substrate.
3. **Supply-chain CI** — build / cosign-sign / SBOM / provenance / Trivy-gate + tool-drift detection.
...
Each domain image is digest-pinned to the base, SBOM'd, provenance-attested
(`mode=max`), **Trivy-gated on fixable CRITICAL CVEs before publish**, and cosign-signed
(keyless, Sigstore/OIDC). `dogfood-scan.yml` builds the whole stack from source and runs
it against this repo on every push/PR; `self-scan.yml` runs the reusable `security-scan`
against this repo.
