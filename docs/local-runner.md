...
[`bin/skill-gate`](../bin/skill-gate) detects the first *functional* runtime (`docker` → `podman` →
`wslc`) and runs the gate against a path:
...
**Verified working** — pull from GHCR, bind-mount a Windows path, run the gate:
...
**Preview status — read this.** WSLC is a **public preview** (GA planned **fall 2026**). The local
...
WSLC's default `virtiofs` file driver makes Windows↔Linux file access **~2× faster** — worth noting
